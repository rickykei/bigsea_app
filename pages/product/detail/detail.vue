	...
	<view class="good-detail-modal" :data-theme="theme()" :class="theme() || ''">
		...
		<view class="tc  header" :style="topBarHeight() == 0 ? '' : 'height:' + topBarHeight() + 'px;padding-top:' + topBarTop() + 'px'">
			<view class="left-icon pr" :style="topBarHeight() == 0 ? '' : 'height:' + topBarHeight() + 'px;'">
				<image class="reg180" @click="goback" src="/static/icon/back-jianatou.png" mode="" style="width: 48rpx;height: 48rpx;"></image>
			</view>
		</view>
		<!-- #endif -->
		<!-- 商品详情模态框 begin -->
		<view class="cover"><image v-if="detail.product_image" :src="detail.product_image" class="image" mode="aspectFill"></image></view>
		<view class="bg-white">
			...
			<view class="wrapper">
				<view class="properties">
					<view class="property" v-if="detail.spec_type == 20">
						...
						<view class="values">
							<view class="value" @click="selecedtSpec(value, key)" v-for="(value, key) in detail.sku" :key="key" :class="{ default: value.checked }">
								{{ value.spec_name }}
							</view>
						</view>
					</view>
					<view class="property" v-if="detail.product_attr.length > 0" v-for="(item, index) in detail.product_attr" :key="index">
						<view class="title">
						...
						<view class="values">
							<view
								class="value"
								@click="selectAttr(value, key, index)"
								v-if="value != ''"
					...
					<view class="property" v-if="detail.product_feed.length > 0">
						...
						<view class="values">
							<view
								class="value"
								@click="selectFeed(item, index)"
								v-for="(item, index) in detail.product_feed"
								:key="index"
